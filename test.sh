...
docker run -i --rm --name some-buildpack quay.io/ibmz/buildpack-deps:bionic-scm apt-get update -y | grep s390x
docker rmi quay.io/ibmz/buildpack-deps:bionic-scm
